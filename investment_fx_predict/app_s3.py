...
def read_dataframe(s3_key, **kwargs):
    s3 = get_client()
    obj = s3.get_object(Bucket=os.environ["AWS_S3_BUCKET"], Key=os.environ["AWS_S3_BASE_PATH"]+s3_key+".gz")
...
def write_dataframe(df, s3_key):
    with io.StringIO() as buf:
        ...
        s3.put_object(
            ...
            Key=os.environ["AWS_S3_BASE_PATH"]+s3_key+".gz",
            Body=io.BytesIO(gz.compress(buf.getvalue().encode()))
        )


def write_sklearn_model(clf, s3_key):
    with io.BytesIO() as buf:
        joblib.dump(clf, buf, compress=9)
        s3 = get_client()
        s3.put_object(
            Bucket=os.environ["AWS_S3_BUCKET"],
            Key=os.environ["AWS_S3_BASE_PATH"]+s3_key,
...
def read_sklearn_model(s3_key):
    s3 = get_client()
    obj = s3.get_object(Bucket=os.environ["AWS_S3_BUCKET"], Key=os.environ["AWS_S3_BASE_PATH"]+s3_key)
    with io.BytesIO(obj["Body"].read()) as buf:
        clf = joblib.load(buf)
...
def write_keras_model(model, s3_key_prefix):
    s3 = get_client()
    s3.put_object(
        Bucket=os.environ["AWS_S3_BUCKET"],
...
def read_keras_model(s3_key_prefix):
    s3 = get_client()
    obj = s3.get_object(Bucket=os.environ["AWS_S3_BUCKET"], Key=s3_key_prefix+".json")
    model = model_from_json(obj["Body"].read())

    obj = s3.get_object(Bucket=os.environ["AWS_S3_BUCKET"], Key=s3_key_prefix+".hdf5")
    with io.BytesIO(obj["Body"].read()) as buf:
...
def write_images(np_array, s3_key, file_name_prefix):
...
def extract_images(df_target_train, df_target_test, s3_key, file_name_prefix):
    tmp_dir = tempfile.TemporaryDirectory()

    s3 = get_client()
    obj = s3.get_object(Bucket=os.environ["AWS_S3_BUCKET"], Key=s3_key)
    with io.BytesIO(obj["Body"].read()) as buf:

...
class LearnEnv():
    ...
    def observe(self):
        obs = np.array(
            [self.df_action.at[self.current_id + i, "opening_price"] for i in range(-5, 6)],
            dtype=np.float32
        )
        obs = np.append(obs, np.array(
            [self.df_action.at[self.current_id + i, "close_price"] for i in range(-5, 6)],
            dtype=np.float32
        ))
...
def build_env(df):
    env = LearnEnv(df, 19080-250, 19080)

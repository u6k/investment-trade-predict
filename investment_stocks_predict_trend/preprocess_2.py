...
def execute():
    ...
    for ticker_symbol in df_companies.index:
        ...
        try:
            ...
            df_data_train, df_data_test, df_target_train, df_target_test = preprocess(
                df_prices, train_start_date, train_end_date, test_start_date, test_end_date)
...
def preprocess(df_prices, train_start_date, train_end_date, test_start_date, test_end_date):
    ...
    df_data_train = df.loc[train_start_id-1: train_end_id-1].drop(["date", "profit_rate", "profit_flag"], axis=1)
    df_data_test = df.loc[test_start_id-1: test_end_id-1].drop(["date", "profit_rate", "profit_flag"], axis=1)
    df_target_train = df.loc[train_start_id: train_end_id][["profit_rate", "profit_flag"]]
    df_target_test = df.loc[test_start_id: test_end_id][["profit_rate", "profit_flag"]]

    return df_data_train, df_data_test, df_target_train, df_target_test

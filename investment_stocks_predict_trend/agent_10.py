...
def execute(experiment=None, max_episode=500):
    ...
    for i in range(1, max_episode+1):
        ...
        if i % 100 == 0:
            ...
            build_figure_result(df_result, i, experiment)
...
def preprocessing():
    ...
    df = df.assign(change=(df["close_price"] / df["opening_price"]))
...
class LearnEnv():
    ...
    def step(self, action):
        ...
        if self.buy_price == 0.0:
            ...
            self.assets = self.funds + self.df_action.at[self.current_id, "close_price"]
    ...
    def observe(self):
        obs = np.array(
            [self.df_action.at[self.current_id-i, "change"] for i in range(1, 21)],
            dtype=np.float32
        )
...
def build_figure_result(df_result, step, experiment=None):
    ...
    if experiment is not None:
        experiment.log_figure(figure_name="win_vs_lose_"+str(step), figure=fig)
    ...
    if experiment is not None:
        experiment.log_figure(figure_name="reward_"+str(step), figure=fig)
    ...
    if experiment is not None:
        experiment.log_figure(figure_name="assets_"+str(step), figure=fig)
